...
import { AbiItem, EthArg, payloadToABI, payloadToMintABI } from "./utils/abi";
import {
    getLockAsset,
    getLockGateway,
    getMintGateway,
...
export const EthereumConfigMap: EvmNetworkConfigMap = {
        ...
        ContractChain<InputContractCall, OutputContractCall, ContractCall>
{
    ...
    public name = Ethereum.chain;

    public feeAsset: string = "ETH";

    public logRequestLimit: number | undefined = undefined;
    private logger: Logger | undefined;

    public static configMap: EvmNetworkConfigMap = EthereumConfigMap;
    public configMap: EvmNetworkConfigMap = EthereumConfigMap;
    ...
    constructor(
        ...
        this.network = this.resolveChainNetwork(renNetwork);
        this.logger = config.logger;
        this.logRequestLimit = config.logRequestLimit;
    ...
    public resolveChainNetwork = (
        renNetwork: EvmNetworkInput,
    ): EvmNetworkConfig => {
        if (!renNetwork) {
            const defaultNetwork =
                this.configMap[RenNetwork.Mainnet] ||
                this.configMap[RenNetwork.Testnet] ||
                this.configMap[RenNetwork.Devnet];
            if (!defaultNetwork) {
        ...
        if (renNetwork && isEvmConfig(renNetwork)) {
            networkConfig = renNetwork;
        } else {
            networkConfig = this.configMap[renNetwork];
        }
    ...
    public transactionExplorerLink = (transaction: ChainTransaction): string =>
        this.network.explorer.transaction(this.transactionHash(transaction));
