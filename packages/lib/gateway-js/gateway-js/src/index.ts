...
const getFrame = () => getElement("_renFrame")
...
const GatewayJS = {
    ...
    close: () => {
        try {
            const renElement = GatewayJS.getPopup();
    ...
    open: async (params: Commitment) => new Promise((resolve, reject) => {

        // Check that GatewayJS isn't already open
        let existingPopup;
        try { existingPopup = GatewayJS.getPopup(); } catch (error) { /* Ignore error */ }
