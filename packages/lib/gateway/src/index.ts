...
export class Gateway {
    ...
    private blocknative: BlocknativeSdk | undefined;
    ...
    public readonly pause = () => {
        this._pause();
        this._sendMessage(GatewayMessageType.Pause, {}).catch(
            this.logger.error
        );
        return this;
    };
    ...
    public readonly resume = () => {
        this._resume();
        this._sendMessage(GatewayMessageType.Resume, {}).catch(
            this.logger.error
        );
        return this;
    };
        ...
        new Promise<Map<string, HistoryEvent>>((resolve, reject) => {
            ...
            listener = (e: { readonly data: GatewayMessage<any> }) => {
                if (
                    ...
                    this._acknowledgeMessage(e.data, {}, popup).catch(
                        this.logger.error
                    );
                    ...
                    switch (e.data.type) {
                        case GatewayMessageType.Ready:
                            if (popup) {
                                this._sendMessage(
                                    GatewayMessageType.GetTransfers,
                                    {},
                                    popup
    ...
    public readonly _open = (
        ...
        (async () => {
            ...
            const provider: Web3Provider =
                web3Provider || (transferParams as TransferParams).web3Provider;
            if (provider !== null) {
            ...
            if ((transferParams as SendParams).sendAmount) {
                ...
                transferParams = resolveSendCall(
                    stringToNetwork(this.network),
                    transferParams as SendParams
                );
            }
            ...
            const popup = createElementFromHTML(
                RenElementHTML(this.id, endpoint, this.isPaused)
            );
        ...
        if (e.data && e.data.from === "ren" && e.data.frameID === this.id) {
            ...
            switch (e.data.type) {
                case GatewayMessageType.Ready:
                    ...
                    const transferParamsFixed = prepareParamsForSendMessage(
                        transferParams
                    );
                ...
                case GatewayMessageType.Status:
                    this._acknowledgeMessage(e.data).catch(this.logger.error);
                    const { status, details } = (e.data as GatewayMessage<
                ...
                case GatewayMessageType.TransferUpdated:
                    this._acknowledgeMessage(e.data).catch(this.logger.error);
                    const { transfer } = (e.data as GatewayMessage<
                ...
                case GatewayMessageType.Cancel:
                    ...
                    if (this.isCancelling) {
                        ...
                        // tslint:disable-next-line: no-object-mutation
                        this.isCancelling = false;
                        this.promiEvent.reject(
                            new Error("Transfer cancelled by user")
                        );
                        return;
                    }
                ...
                case GatewayMessageType.GetEthereumTxStatus:
                    (async () => {
                        try {
                            ...
                            const txHash = (e.data
                                .payload as GatewayMessagePayload<
                ...
                case GatewayMessageType.FindMintTransaction:
                    (async () => {
                        try {
                            ...
                            >;
                            const txHash = await findTransactionBySigHash(
                                this.network,
                                this.web3,
                                token,
                                sigHash,
                ...
                case GatewayMessageType.RequestNotificationPermission:
                    this._acknowledgeMessage(e.data, {}).catch(
                        this.logger.error
                    );
                    if (!Push.Permission.has()) {
                        this._getNotificationButton().classList.add(
                ...
                case GatewayMessageType.ShowNotification:
                    ...
                    const { title, body } = e.data
                        .payload as GatewayMessagePayload<
                        GatewayMessageType.ShowNotification
                    >;
                    try {
                        if (Push.Permission.has()) {
                            ...
                            Push.create(title, {
                                body,
                                icon:
                                    "https://gateway.renproject.io/favicon.ico",
                                timeout: 4000,
                                tag,
                                onClick: () => {
                                    window.focus();
    ...
    private readonly _sendMessage = async <Type extends GatewayMessageType>(
        ...
        new Promise<any>(async (resolve) => {
            ...
            while (!acknowledged && contentWindow) {
                if (count >= 1000) {
                    throw new Error(
                        `Lost communication with Gateway iFrame - unable post message.`
                    );
                }

                count++;
                const gatewayMessage: GatewayMessage<Type> = {
                    from: "ren",
                    frameID: this.id,
                    type,
                    payload,
    ...
    private readonly _acknowledgeMessage = async <
        Type extends GatewayMessageType
    >(
        message: GatewayMessage<Type>,
        payload?: GatewayMessageResponse<Type>,
        iframeIn?: ChildNode | null
    ) => {
        ...
        const response: GatewayMessage<GatewayMessageType.Acknowledgement> = {
            from: "ren",
    ...
    private readonly _getOrCreateGatewayContainer = () => {
        ...
        const body: ReadonlyArray<HTMLBodyElement | HTMLHtmlElement> = [
...
export default class GatewayJS {
    ...
    public static readonly utils: typeof utils & {
        useBrowserWeb3: typeof useBrowserWeb3;
    } = { ...utils, useBrowserWeb3 };
    ...
    constructor(network?: RenNetwork | string, config?: GatewayJSConfig) {
        const publicNetworks: readonly RenNetwork[] = [
            RenNetwork.Mainnet,
            RenNetwork.Chaosnet,
            RenNetwork.Testnet,
        ];
        if (typeof network === "string") {
            validateString<RenNetwork>(
                network,
                `Invalid network. Expected one of ${publicNetworks.join(", ")}`,
                Object.values(RenNetwork) as readonly RenNetwork[]
            );
        }
     ...
     */
    public readonly getGateways = async (options?: {
        all: boolean;
    }): Promise<Map<string, HistoryEvent>> => {
        const gateways = await new Gateway(
            this.network,
            this.config
        )._getGateways();
     ...
     * @returns {Gateway}
     */
    public readonly lockAndMint = (
        params: LockAndMintParams | LockAndMintParamsSimple | SendParams,
        uniqueID?: string
    ): Gateway => {
        if (
            (params as SendParams).sendTo &&
            !(params as LockAndMintParamsSimple).contractFn
        ) {
            ...
            const {
                ...
                txConfig,
                ...restOfParams
            } = params as LockAndMintParamsSimple;
            params = {
                ...restOfParams,
                contractCalls: [
    ...
    public readonly burnAndRelease = (
        params: BurnAndReleaseParams | BurnAndReleaseParamsSimple | SendParams,
        uniqueID?: string
    ): Gateway => {
        if (
            (params as SendParams).sendTo &&
            !(params as BurnAndReleaseParamsSimple).contractFn
        ) {
            params = resolveSendCall(this.network, params as SendParams);
        } else if ((params as LockAndMintParamsSimple).sendTo) {
            const {
                ...
                txConfig,
                ...restOfParams
            } = params as BurnAndReleaseParamsSimple;
            params = {
                ...restOfParams,
                contractCalls: [
                    { sendTo, contractFn, contractParams, txConfig },
    ...
    public readonly open = (
        ...
        if ((params as LockAndMintEvent).eventType === EventType.LockAndMint) {
            return this.recoverTransfer(
                (undefined as unknown) as Web3Provider,
                params as LockAndMintEvent | BurnAndReleaseEvent
            );
        }
        ...
        if (sendToken === "BTC" || sendToken === "ZEC" || sendToken === "BCH") {
            throw new Error(
                `Ambiguous token ${sendToken} - call "lockAndMint" or "burnAndRelease" instead of "open"`
            );
        }
        if (parseRenContract(sendToken).to === Chain.Ethereum) {
            return this.lockAndMint(params as LockAndMintParams, uniqueID);
        } else {
            return this.burnAndRelease(
                params as BurnAndReleaseParams,
                uniqueID
            );
        }
    };
    ...
    public readonly recoverTransfer = (
        web3Provider: Web3Provider,
        params: LockAndMintEvent | BurnAndReleaseEvent,
        uniqueID?: string
    ): Gateway => {
        return new Gateway(
            this.network,
