...
import { Ox } from "@renproject/interfaces";

import { bchAddressFrom, bchAddressToHex, createBCHAddress } from "../../src/bch";
import { btcAddressFrom, btcAddressToHex, createBTCAddress } from "../../src/btc";
import { createZECAddress } from "../../src/zec";
        ...
        [
            ...
            .forEach(({ network, expected, hex, actual }) => {
                it(network.name, async () => {
                    if (expected) {
                        const address = createBTCAddress(network.isTestnet, network.contracts.renVM.mpkh, "0x1234");
                        address.should.equal(expected);
                        createBTCAddress(network.isTestnet, network.contracts.renVM.mpkh, "1234")
...
describe("zec.ts", () => {
    describe("createZECAddress", async () => {
        [
            ...
            { network: NetworkChaosnet, expected: "t3WWGCowA5cEDR8GHrVpV67BTUcqH6DxrAf", get hex() { return Ox(decode(this.expected)); } },
...
describe("bch.ts", () => {
    describe("createBCHAddress", async () => {
        [
            ...
            { network: NetworkChaosnet, expected: "bitcoincash:pzp0aa0axmcnqhntqe40mhqwkz59hy4gsyszrstuel", get hex() { return Ox(Buffer.from(this.expected)); } },
