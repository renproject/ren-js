...
export class RenVMProvider implements Provider<RenVMParams, RenVMResponses> {
    ...
    public submitBurn = async (
        ...
        const response = await this.provider.sendMessage(
            RPCMethod.MethodSubmitTx,
            {
                tx: {
                    to: renContract,
                    in: [{ name: "ref", type: RenVMType.TypeU64, value: ref }],
                },
                tags,
            }
        );
    ...
    public readonly queryMintOrBurn = async <
        T extends ResponseQueryMintTx | ResponseQueryBurnTx
    >(
        utxoTxHash: string
    ): Promise<T> => {
        return (await this.provider.sendMessage(RPCMethod.MethodQueryTx, {
...
export const defaultProvider = (
    ...
    try {
        const provider = new MultiProvider<RenVMParams, RenVMResponses>(
            [rpcUrl],
            logger
        ) as Provider<RenVMParams, RenVMResponses>;
        return new RenVMProvider(network, provider, logger);
    } catch (error) {
        if (String(error && error.message).match(/Invalid node URL/)) {
            throw new Error(`Invalid network or provider URL: "${network}"`);
        }
        throw error;
    }
};
