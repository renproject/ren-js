...
describe("burnAndRelease response is returned correctly", () => {
    it.skip("Chaosnet", async () => {
        ...
        (
            await renJS
                .burnAndRelease({ sendToken: "BTC", burnReference: 0x133 })
                .submit()
        ).in.to.should.equal("bc1q2cylsj8dvrhyd3lnvy8xz6lk604up0rk0ps72x");

        (
            await renJS
                .burnAndRelease({ sendToken: "ZEC", burnReference: 0x5f })
                .submit()
        ).in.to.should.equal("t1eB5xqXANzuBdpiWr8u4MSRcgKggXYydc3");

        (
            await renJS
                .burnAndRelease({ sendToken: "BCH", burnReference: 0x54 })
                .submit()
        ).in.to.should.equal(
    ...
    it.skip("Testnet", async () => {
        ...
        (
            await renJS
                .burnAndRelease({ sendToken: "BTC", burnReference: 0x1 })
                .submit()
        ).in.to.should.equal("n25GA3GvGdu9MRAE16WgPBn1UmAaQ1DEws");

        (
            await renJS
                .burnAndRelease({ sendToken: "ZEC", burnReference: 0x9c })
                .submit()
        ).in.to.should.equal("tmMsZTMQAakgna4B9wntQUppMgEaCk5Deuc");

        (
            await renJS
                .burnAndRelease({ sendToken: "BCH", burnReference: 0x87 })
                .submit()
        ).in.to.should.equal(
    ...
    it.skip("Devnet", async () => {
        ...
        (
            await renJS
                .burnAndRelease({ sendToken: "BTC", burnReference: 0x2b })
                .submit()
        ).in.to.should.equal("n25GA3GvGdu9MRAE16WgPBn1UmAaQ1DEws");

        (
            await renJS
                .burnAndRelease({ sendToken: "ZEC", burnReference: 0x7e5 })
                .submit()
        ).in.to.should.equal("tmK29k59HSB8ztLUox2q15jrC9LZtuLsCNA");

        (
            await renJS
                .burnAndRelease({ sendToken: "BCH", burnReference: 0x9d })
                .submit()
        ).in.to.should.equal(
