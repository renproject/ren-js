...
export class LocalStorageProvider<V = HistoryEvent> implements StorageProvider<string, V> {
...
export class MemoryProvider<K, V> implements StorageProvider<K, V> {
    ...
    set = (k: K, v: V) => { this.store.set(k, v); return v; };
...
const useTransferContainer = () => {
    ...
    const [store, setStore] = useState<StorageProvider<string, HistoryEvent> | undefined>();
    ...
    const getTransfers = useCallback(async () => {
        ...
        if (store) {
            ...
            transfers = await keys.reduce(async (mapP, key) => {
                ...
                return transfer && transfer.transferParams && !transfer.archived && !transfer.returned ?
    ...
    return {
        store, noLocalStorage, connect, getTransfers,
    };
};
