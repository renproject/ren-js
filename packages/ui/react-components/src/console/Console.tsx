    ...
    public render = (): JSX.Element => {
        ...
        return (
            <div
                role="log"
                {...props}
                className={["console", className].join(" ")}
            >
                {(logs as LogEntry[]).map((e) => (
                    <span>{e}</span>
                ))}
                <br
                    className="console--bottom"
                    ref={this.updateBottomElement}
                />
            </div>
        );
    };

    public updateBottomElement = (ref: HTMLBRElement | null) => {
        this.bottomElement = ref;
    };
