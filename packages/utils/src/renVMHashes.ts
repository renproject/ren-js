...
export const renVMHashToBase64 = (txHash: string) => {
    ...
    if (/^(0x)?[0-9a-fA-Z]{64}$/.exec(txHash)) {
        return toURLBase64(fromHex(txHash));
    }
    // Already base64 - ensure it's in URL-base64 format.
    return toURLBase64(fromBase64(txHash));
};
